...
const RecommendRecipes = () => {
    ...
    const onClickHandler = async (e) => {
        ...
        try {
            const resp = await axios.post("http://3.36.56.125/api/recipes/recommend",{
                base : "닭",
                foods : ["닭","",""]
    ...
    return (
        <StyledRecipesWrapper>
            ...
            <StyledButtonWrapper>
                {warningIngredient && warningIngredient.map((data, index)=> (
                    <StyledButtonScroll
                    key={index}>
                        <StyledIngredientButton
                        onClick={onClickHandler}
                        value={data.food_name}
                        >{data.food_name}</StyledIngredientButton>
                    </StyledButtonScroll>
                ))}
                <StyledRecipes>
